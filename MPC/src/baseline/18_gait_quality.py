...
사용:
  .venv/Scripts/python.exe MPC/src/baseline/18_gait_quality.py --vx 0.5 0.7 --var "tds=1.25" "tds=1.25,dum=1e-3"
"""
...
def _need(modname, folder):
    """ctl= 컨트롤러 모듈 불러오기. 이 폴더에 없으면 어느 폴더에서 돌려야 하는지 알려 준다 (9/24 src 분리)."""
    try:
        return importlib.import_module(modname)
    except ModuleNotFoundError as e:
        if e.name != modname:
            raise
        raise RuntimeError(f"ctl={folder} 는 MPC/src/{folder}/18_gait_quality.py 로 실행하세요 "
                         f"({modname}.py 가 이 폴더에 없음)")
...
def make_ctor(kw):
    ...
    if name == "affine":
        mod = _need("23_walk_affine", "affine")
